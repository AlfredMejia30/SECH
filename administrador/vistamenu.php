...
$stmt = $pdo->prepare('SELECT m.nombre, mg.id_grupo FROM materias m
                       INNER JOIN materia_grupo mg ON m.id = mg.id_materia 
                       WHERE m.id = ?');
$stmt->execute([$idMateria]);
$materia = $stmt->fetch();
$nombreMateria = $materia['nombre']; // Aquí está el nombre de la materia
$idGrupo = $materia['id_grupo']; // Aquí está el ID del grupo
...
<body >
 <div class="contanier text-center encabezado">
  ...
  <a class="nav-link " href="#" onclick="getMateriaId('./vista_materias.php', '<?= $idGrupo ?>')">Atras</a>
  ...
  <button class="btn btn-primary btn-sm botoncito2" name="unidades" onclick="getMateriaId('unidades.php', '<?= $idMateria ?>')">Unidades</button>
<button class="btn btn-primary btn-sm botoncito2" name="General" onclick="getMateriaId('repDirectora.php', '<?= $idMateria ?>')">General</button>



<script>
function getMateriaId(destination, id) {
  window.location.href = destination + "?id=" + id;
}

function getGrupoId(destination, id) {
  window.location.href = destination + "?idGrupo=" + id;
}
</script>
